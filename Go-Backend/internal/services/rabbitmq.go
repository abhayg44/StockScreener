...
func GetStockFromRabbitmq() (models.StockData, error) {
    ...
    for {
        ...
        case d:=<-msgs:
            ...
            max_iterations = 5
            if stockData.LastUpdated.Sub(time.Now()) > -2*time.Minute && max_iterations > 0 {
                api.RefreshCurrentStockDataJob()
                max_iterations--
                fmt.Println("Data not updated yet, retrying... remaining attempts: ", max_iterations)
            } else {
